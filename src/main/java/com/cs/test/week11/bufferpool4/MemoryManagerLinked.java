...
/**
 * 该内存管理对象主要是在当你需要长时间的new一快内存的时候使用， （主要作用是为了不让GC对这些内存不停的释放分配而消耗性能）
 * 与MomoryManagerByte的区别是在产生内存碎片的几率稍大一点，做法是当你获取一个指定的内存时 我们会到该大小对应的内存块队列中去找，
 * 如果没有在创建该内存块，并放进该内存快的队列当中,如果每次获取的内存大小不同则会每次都创建新的内存块队列。 注意该对象管理类
 * 在初始化的时候不会预先分配，只有当需要使用的时候才会分配，并保留下来
 */
public class MemoryManagerLinked {
	/** 是否在虚拟机所所管理的范围内创建内存 */
	...
	public ByteBuffer allocate() {
		return allocate(this.defaultSize);
	}
	...
	public static void main(String[] args) {
		try {
			...
			ByteBuffer buf_1 = memoryManagerLinked.allocate(1);
			ByteBuffer buf_2_1 = memoryManagerLinked.allocate(2);
			ByteBuffer buf_2_2 = memoryManagerLinked.allocate(2);
			ByteBuffer buf_3_1 = memoryManagerLinked.allocate(3);
			ByteBuffer buf_3_2 = memoryManagerLinked.allocate(3);
			ByteBuffer buf_3_3 = memoryManagerLinked.allocate(3);
			ByteBuffer buf_3_4 = memoryManagerLinked.allocate(3);
